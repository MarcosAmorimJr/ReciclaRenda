...
class ReciclaRendaWeb {
    constructor() {
        ...
        this.API_BASE = (window.location.hostname.includes('vercel.app') || !window.location.hostname.includes('localhost'))
    ...
    initializeMockData() {
        return {
            wasteTypes: {
                ...
                plastico: { name: 'Plástico', pricePerKg: 1.40, co2Factor: 2.8, icon: '♻️' },
                metal: { name: 'Metal', pricePerKg: 6.20, co2Factor: 8.1, icon: '🥇' },
                vidro: { name: 'Vidro', pricePerKg: 0.25, co2Factor: 1.2, icon: '🍾' },
                eletronicos: { name: 'Eletrônicos', pricePerKg: 28.00, co2Factor: 25.5, icon: '📱' },
                moveis: { name: 'Móveis', pricePerKg: 0, co2Factor: 15.0, icon: '🪑' },
                oleo: { name: 'Óleo de Cozinha', pricePerKg: 1.80, co2Factor: 2.5, icon: '🛢️' }
            },
            collectionPoints: [
                {
                    id: 1,
    ...
    async loadCollectionPoints() {
        ...
        // Tentar carregar pontos reais da API
        if (this.useRealAPI) {
            try {
            ...
            <div class="point-card" data-type="${point.type}">
                ...
                <div class="point-materials">
                    ${point.materials.map(material => {
                        const wasteType = this.mockData.wasteTypes[material];
                        return wasteType ? `<span class="material-tag">${wasteType.name}</span>` : '';
                    }).join('')}
                </div>
